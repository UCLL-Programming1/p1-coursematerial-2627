...
def make_pile(top, below=()):
    ...
    current suit yet: play_turn still does that."""
...
def test_function_play_turn_eight(pytestconfig):
    ...
    assert values(pile.cards) == ["CK", "H8"], (
        f"The played 8 should be on top of the discard pile, "
        f"but the pile holds {pile.cards!r}."
    )
    assert result == "D", (
